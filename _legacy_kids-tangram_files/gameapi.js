...
window.matchMedia || (window.matchMedia = function(c) {
	var a = c.document, w = a.documentElement, l = [], t = 0, x = "", h = {}, G = /\s*(only|not)?\s*(screen|print|[a-z\-]+)\s*(and)?\s*/i, H = /^\s*\(\s*(-[a-z]+-)?(min-|max-)?([a-z\-]+)\s*(:?\s*([0-9]+(\.[0-9]+)?|portrait|landscape)(px|em|dppx|dpcm|rem|%|in|cm|mm|ex|pt|pc|\/([0-9]+(\.[0-9]+)?))?)?\s*\)\s*$/, y = 0, A = function(b) {
			...
			y = setTimeout(function() {
				...
				if (0 <= a) {
					B();
					do if (b = l[e - a])if ((j = A(b.mql.media)) && !b.mql.matches || !j && b.mql.matches)if (b.mql.matches = j, b.listeners)for (var j = 0, g = b.listeners.length; j < g; j++)b.listeners[j] && b.listeners[j].call(c, b.mql); while (a--)
				}
			}, 10)
...
function mediahack(a, i) {
	function n(a, i) {
		var n = i.split(" "), l = a.classList;
		if (l)for (var i, e = 0; e < n.length; e++)(i = n[e]) && l.add(i); else {
			for (var i, t = " " + a.className + " ", r = t, e = 0; e < n.length; e++)(i = n[e]) && (i += " ", ~t.indexOf(" " + i) || (t += i));
			t !== r && (a.className = t.slice(1, -1))
		}
	}

	function l(a, i) {
		var n = i.split(" "), l = a.classList;
		if (l)for (var i, e = 0; e < n.length; e++)(i = n[e]) && l.remove(i); else {
			for (var i, t = " " + a.className + " ", r = t, e = 0; e < n.length; e++)(i = n[e]) && (i = " " + i + " ", ~t.indexOf(i) && (t = t.replace(i, " ")));
			t !== r && (a.className = t.slice(1, -1))
		}
	}
	...
	for (var e in i) {
		var t = window.matchMedia(i[e]);
		!function(i, e) {
			var t = function(i) {
				i.matches ? n(a, e) : l(a, e)
			};
			t(i), i.addListener(t)
		}(t, e)
	}
...
fg_api.prototype.sdkModule = function() {
	...
	module.prototype.init = function() {
		...
		if (window.top != window.self && !hasLocalStorageKeys()) window.top.postMessage({
			scope: 'famobi_sdk',
			method: 'getData',
			time: +(new Date),
...
fg_api.prototype.screenshotModule = function() {
	...
	module.prototype.init = function() {
		...
		if (!self.config.gameParams.screenshot || !self.config.gameParams.screenshot.active || !Object.keys(self.config.gameParams.screenshot.areas).length
		) {
			return false;
		}
...
fg_api.prototype.orientationModule = function() {
	...
	orientationPrototype.update = function(orientation) {
		...
		if (typeof orientation != 'undefined' && !detection.is.pc && !detection.is.tablet && window.screen && window.screen.height !== window.screen.width) {
	...
	flashPrototype.init = function() {
		...
		for (var key in arr) {
			...
			for (var attrName in self.config.gameParams.flash[arr[key]]) {
				M[arr[key]][attrName] = self.config.gameParams.flash[arr[key]][attrName];
			}
		}
